        ...
        if (existingEmail) {
            ...
            transporter
                ...
                .then(async() => {
                    ...
                    setTimeout(async() => {
                        const otpupdate = await OtpModel.findOneAndUpdate({ _id: otpsave._id }, { otp: otp, status: false });
                    }, 100000);
...
// Update emergency contacts and location function
router.post("/update-profile", userAuth, async(req, res) => {
    try {
        const userId = req.user._id;
        const { emergencyContacts, permanentLocation } = req.body;
        ...
        for (let contact of emergencyContacts) {
            if (!contact.contactName || !contact.contactNumber || !contact.contactEmail) {
                return res.status(400).send({
                    status: "Failed",
                    message: "Each emergency contact must have a name, number, and email address",
                });
            }
        }

        // Validate permanentLocation
        if (!permanentLocation ||
            typeof permanentLocation.latitude !== "number" ||
            typeof permanentLocation.longitude !== "number"
        ) {
            return res.status(400).send({
                status: "Failed",
                message: "Invalid permanent location data",
            });
        }
        ...
        user.permanentLocation = permanentLocation;
        await user.save();
